...
- **Compaction dropped earlier prompts:** for the prompt-archive step, only offer prompts you can still see verbatim. Don't reconstruct from summaries. If the current session is the one that did the work but compaction dropped it, the on-disk transcript at `~/.claude/projects/<sanitized-cwd>/<this-session-uuid>.jsonl` still holds the original turns; treat it as a prior-session source (see `references/prior-session-detection.md`).
- **Compaction dropped the session's tool-call history:** path detection (Step 0) cannot rely on it. Check the on-disk transcripts first (see `references/prior-session-detection.md`); only ask the user as a last resort.
- **Prior session did the work, this session is fresh:** invoked `/commit` after exiting and reopening, with staged or untracked Claude-touched files present. Run the prior-session search in `references/prior-session-detection.md`; do not silently fall through to the human-only path. Fresh repo (no commits) + untracked files is the strongest signal.
